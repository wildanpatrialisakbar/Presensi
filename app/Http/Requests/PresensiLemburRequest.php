...
class PresensiLemburRequest extends FormRequest
{
     ...
     * @throws ValidationException
     */
    public function ensureThatThisIsTheTimeToPresensiLembur($waktu)
    {
        $waktu = Carbon::create($waktu);

        if (!$waktu->greaterThan(Carbon::create($this->getConfiguration()->presensi_pulang_sampai))) {
            throw ValidationException::withMessages([
                'message' => 'Bukan waktu presensi lembur.'
            ]);
        }
    }
